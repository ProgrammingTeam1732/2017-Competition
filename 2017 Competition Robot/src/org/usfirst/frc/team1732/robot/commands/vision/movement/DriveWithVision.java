...
public class DriveWithVision extends Command {
	...
	driveTrain.mainController.setSetpoint(MAX_SETPOINT);
	driveTrain.mainController.setPIDValues(DriveTrain.normalPID.changePID(0.02, 0, 0));
	driveTrain.gyroController.setSetpoint(0);
	...
	if (visionMain.canSeeGearPeg() && visionMain.isNewGearDataAvailable()) {
		...
		leftSetpoint = MAX_SETPOINT - driveTrain.leftEncoder.getDistance();
	    if (rightSetpoint > MAX_SETPOINT)
		rightSetpoint = MAX_SETPOINT - driveTrain.rightEncoder.getDistance();
	    driveTrain.mainController.adjustSetpoint(leftSetpoint, rightSetpoint);
	}

	double leftOutput = driveTrain.mainController.left.getOutput();
	double rightOutput = driveTrain.mainController.left.getOutput();
	...
	return (driveTrain.mainController.areBothErrorNegative() || driveTrain.mainController.areBothOnTarget())
		|| isTimedOut();
    }
