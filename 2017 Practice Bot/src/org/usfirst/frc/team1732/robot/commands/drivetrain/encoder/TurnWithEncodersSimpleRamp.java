...
public class TurnWithEncodersSimpleRamp extends Command {
    ...
    public TurnWithEncodersSimpleRamp(DoubleSupplier angle, double stopRampPercent, double stopFlatPercent) {
    ...
    public TurnWithEncodersSimpleRamp(double angle, double stopRampPercent, double stopFlatPercent) {
	...
	if (percentCompleted < stopRampPercentage) {
		...
		left = RAMP_RATE * Math.signum(prevLeft) + prevLeft;
	    }
	    if (Math.abs(prevRight - right) > RAMP_RATE) {
		right = RAMP_RATE * Math.signum(prevRight) + prevRight;
	    }
