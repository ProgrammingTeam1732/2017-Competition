...
public class VisionThread implements Runnable {
	...
	public void run() {
		...
		while (!Thread.interrupted()) {
			...
			/*
			 * if (serialLine != "") { String[] rects = serialLine.split("\n");
			 * Rectangle[] total = new Rectangle[rects.length]; for (int i = 0;
			 * i < rects.length; i++) { String[] data = rects[i].split(" ");
			 * total[i] = new Rectangle(Integer.parseInt(data[1]),
			 * Integer.parseInt(data[3]), Integer.parseInt(data[5]),
